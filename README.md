...
| `scripts/`                                | סקריפטי scraping לאיסוף נתוני זכויות והטבות מאתרי עיריות ת"א וירושלים |
| `nixpacks.toml`                           | הגדרות build של Railway                        |

## פיצ'רים

1. **Dashboard** — כרטיסי קטגוריות עם תרשימי עוגה המציגים את ההתקדמות, ציר זמן כללי ולוח יומי מותאם אישית הכולל משימות שנבחרו להיום.
2. **לוח יומי** — אפשרות להוסיף משימות ספציפיות לרשימת "היום" — בלחיצה על + או בגרירה ישירה של משימה או קטגוריה שלמה אל הלוח; מציג יום ותאריך נוכחיים.
3. **קטלוג משימות** — משימות למעבר דירה מקובצות לפי קטגוריה ושלב (לפני / יום המעבר / אחרי), עם מעקב אחר סטטוס הביצוע, סינון לפי סטטוס, הגדרת מועד, הערות ושלבי ביצוע.
4. **זכויות והטבות** — קטלוג עם אפשרות חיפוש, מסונן לפי פרופיל המשתמש והתחומים שמעניינים אותו.
5. **מסמכים וטפסים** — טפסים ספציפיים לעיר עם מעקב אחר סטטוס המילוי וקישורים ישירים לטפסים הרשמיים.
6. **משימות אישיות** — המשתמש יכול ליצור משימות משלו עם קטגוריות, מועדים ושלבי ביצוע.
7. **סוכן AI למשימות** — סוכן שיכול לקרוא משימות, להוסיף, לשנות סטטוסים ומועדים, ולהציע מה להוסיף ללוח היומי. השיחות נשמרות ומאפשרות חזרה להיסטוריית שיחות קודמות.
8. **סוכן AI למסמכים** — סוכן שעונה על שאלות על טפסים ובירוקרטיה; תומך בהעלאת תמונות לזיהוי מסמכים. גם כאן השיחות נשמרות לעיון עתידי.
9. **בוט Telegram** — לכל משתמש יש id ייעודי; הבוט משמש כערוץ נוסף לקבלת התראות יומיות ומאפשר שיחה עם סוכן המשימות דרך אפליקציית Telegram בנייד.
10. **התראות יומיות** — המשתמש מקבל התראות אוטומטיות כל יום ב-08:00 בבוקר, שעון ישראל. ההתראות כוללות ברכת הצלחה ביום המעבר, מועדים קרובים (3 ימים לפני), 5 המשימות הדחופות מהשלב הנוכחי, והתראות על איחורים במועדים. ההתראות מופיעות גם באתר דרך פעמון התראות עם ספירת הודעות שלא נקראו, וגם נשלחות ב-Telegram אם מקושר.
11. **שאלון פתיחה והתאמה אישית** — תהליך בן 3 שלבים לאיסוף תאריך המעבר, עיר היעד ופרטי פרופיל רלוונטיים (עיסוק, מצב משפחתי, הכנסה, מאפייני זכאות ועוד). המשתמש יכול לעדכן ולערוך את הפרופיל ולקשר/לנתק Telegram בכל עת.
12. **פרסונליזציה** — כל התוכן במערכת מותאם אישית לפי הפרופיל שנאסף בשאלון הפתיחה: קטלוג המשימות מציג רק משימות רלוונטיות לשלב ולעיר היעד של המשתמש; זכויות והטבות מסוננות לפי עיסוק, מצב משפחתי ודגלי זכאות; הטפסים מוצגים בהתאם לעיר; וסוכני ה-AI מקבלים את הפרופיל המלא כהקשר ומתאימים את תשובותיהם בהתאם.

## דאטא בייסים (טבלאות Supabase)

| טבלה                            | תיאור                                                                                    |
| ------------------------------- | ---------------------------------------------------------------------------------------- |
| `user_profiles`                 | שורה אחת לכל משתמש: **כל הפרטים שנאספו בשאלון הפתיחה** — תאריך מעבר, עיר יעד, מאפיינים אישיים, Telegram chat_id |
...
- **Webhook אוטומטי** — בעת עליית השרת, הבוט רושם באופן אוטומטי את כתובת ה-webhook שלו מול Telegram, ללא צורך בהגדרה ידנית.
- **קישור חשבון** — המשתמש בוחר באפליקציה באפשרות "חבר Telegram", מקבל קוד בן 6 ספרות ופותח את קישור הבוט. הבוט מאמת את הקוד ומקשר את חשבון ה-Telegram לחשבון המשתמש באפליקציה.
- **התראות יומיות** — התראות שנוצרות במסגרת תהליך ההתראות היומי נשלחות אוטומטית גם לשיחת ה-Telegram המקושרת של המשתמש.
- **שיחה עם סוכן המשימות** — משתמשים שקישרו את חשבון ה-Telegram יכולים לנהל שיחה עם סוכן המשימות ישירות מתוך Telegram, בנוסף לממשק האפליקציה.

## שירות ההתראות

APScheduler מפעיל את `generate_daily_notifications()` מדי יום ב-**08:00 שעון ישראל (Asia/Jerusalem)**.

עבור כל משתמש שהשלים את שאלון הפתיחה, המשימה מייצרת עד ארבעה סוגי התראות לפי סדר עדיפות:
...
פותחים את http://localhost:5174, מתחברים עם Google, משלימים את שאלון הפתיחה ומגיעים ל-dashboard.
