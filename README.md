...
- **לוח יומי** — אפשרות להוסיף משימות ספציפיות לרשימת "היום" — בלחיצה על + או בגרירה ישירה של משימה או קטגוריה שלמה אל הלוח; מציג יום ותאריך נוכחיים.
- **קטלוג משימות** — משימות למעבר דירה מקובצות לפי קטגוריה ושלב (לפני / יום המעבר / אחרי), עם מעקב אחר סטטוס הביצוע, סינון לפי סטטוס, הגדרת מועד, הערות ושלבי ביצוע.
...
- **סוכן AI למשימות** — סוכן שיכול לקרוא משימות, להוסיף, לשנות סטטוסים ומועדים, ולהציע מה להוסיף ללוח היומי. השיחות נשמרות ומאפשרות חזרה להיסטוריית שיחות קודמות.
- **סוכן AI למסמכים** — סוכן שעונה על שאלות על טפסים ובירוקרטיה; תומך בהעלאת תמונות לזיהוי מסמכים. גם כאן השיחות נשמרות לעיון עתידי.
...
- **התראות יומיות** — המשתמש מקבל התראות אוטומטיות כל יום ב-08:00 בבוקר, שעון ישראל. ההתראות כוללות ברכת הצלחה ביום המעבר, מועדים קרובים (3 ימים לפני), 5 המשימות הדחופות מהשלב הנוכחי, והתראות על איחורים במועדים. ההתראות מופיעות גם באתר דרך פעמון התראות עם ספירת הודעות שלא נקראו, וגם נשלחות ב-Telegram אם מקושר.
...
- **פרסונליזציה** — כל התוכן במערכת מותאם אישית לפי הפרופיל שנאסף בשאלון הפתיחה: קטלוג המשימות מציג רק משימות רלוונטיות לשלב ולעיר היעד של המשתמש; זכויות והטבות מסוננות לפי עיסוק, מצב משפחתי ודגלי זכאות; הטפסים מוצגים בהתאם לעיר; וסוכני ה-AI מקבלים את הפרופיל המלא כהקשר ומתאימים את תשובותיהם בהתאם.
